    ...
    def accountsMerge(self, accounts):
        ...
        for i in range(len(accounts)):
            for j in range(1, len(accounts[i])):
                self.root[accounts[i][j]] = accounts[i][j]
                self.owner[accounts[i][j]] = accounts[i][0]
        for i in range(len(accounts)):
            p = self.find(accounts[i][1], self.root)
            for j in range(2, len(accounts[i])):
                self.root[self.find(accounts[i][j], self.root)] = p
        for i in range(len(accounts)):
            for j in range(1, len(accounts[i])):
                s = self.find(accounts[i][j], self.root)
                if self.tmp.__contains__(s):
                    self.tmp[s].append(accounts[i][j])
                else:
                    self.tmp[s] = [accounts[i][j]]
        for i, j in self.tmp.items():
            j = sorted(list(set(j)))
            j.insert(0, self.owner[i])
            self.ans.append(j)
        return self.ans
    
    def find(self, s, l):
        return s if s == l[s] else self.find(s=l[s], l=l)
...
if __name__ == '__main__':
    ...
    print(s.accountsMerge(accounts=test))
